...
四级或以上标题应尽量避免使用，层级太深，容易导致行文逻辑上的混乱。
...
```markdown
![这里是图片的替代性文字描述](https://image.marapython.com/pexels-frank-cone-2495173.jpg)
```
...
下面这个三阶三次微分方程，在Typora里面渲染正常，Hexo中块状公式的渲染也正常，但行内公式的渲染似乎有点问题，如下：

$\left(y^{'''} \right)^3+3y^{''}+6y'–12=0$，三阶三次

$\left(y^{'''} \right)^3+3y^{''}+6y'–12=0$
