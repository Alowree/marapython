...
toc_number: true
categories: GMAT
abbrlink: penguins-of-madagascar
date: 2021-11-19 20:21:11
updated:
tags:
keywords:
...
Joyous, frolicking, `waddling`,	 `摇摇摆摆的`

cute and `cuddly` life.	`令人想拥抱的`

Look at them tumbling

onto their `chubby` bum-bums.	`圆胖的，丰满的`

Who could take

these `frisky` little snow clowns...	`活泼的，欢闹的`
...
I live on the `flipping` frozen tundra.	`（非正式）该死的，非常的（用于加强语气、表愤怒）` 	冻原，冻土地带
...
300 feet of `kelp`, dry it in the...	`巨藻，海藻，海带`
...
I have `amniotic` sac in my mouth!	`羊膜囊`
...
\- The <u>royal flush</u>.	`皇家同花顺（10/J/Q/K/A）`
...
\- 'Cause I respect a soldier with some `moxie`.	`勇气，力量`
...
\- Rico, sonic `incursion` device.	`入侵，进入`
...
nanny state for those `succulent`	`多汁的，多水分的`

but chemically hazardous

bits of `puffed` heaven called...	`膨化的`

Cheezy Dibbles!

Happy ding-dong birthday,

you little `scamp`!	`（昵称）顽皮的家伙`
...
<u>on the lam</u> the rest of our lives,	`在逃，潜逃`
...
The `penultimate`, plus one.	`倒数第二的`
...
Private, quit `lollygagging`.	`浪费光阴`

And regular `gagging`.	`插科打诨`

Dark and `ominous`... Two of my least	`不祥的，不吉利的`
...
my adoring `legion` of fans.	军团，军队
...
Cream of the corn on a `platinum` cob.	铂，铂金

And we're gonna take your deadly green `goop`	黏糊糊的东西

and `sashay` right out the exit hatch.	滑步而走；到处走动
...
The name's Skipper. I run this `outfit`.	【非正式】（协同工作的）一组人，一队人（尤指乐队、运动队、公司）
...
`sheer` fun of it. No reason at all.	纯粹的，完全的

And then there's Private.

He's... he's sort of our,

you know, secretary/`mascot`.	吉祥物；福神
...
\- Kowalski, cancel our `improv` class!	即兴表演
...
`waddle` of useless penguins!	蹒跚，摇摆的步子
...
Put on your `jammies`, penguin.	睡衣，睡裤 pajamas
...
your turn to pick up the <u>slack</u>.	（绳索的）松驰部分
...
\- Rub the angry out of the `tummy`.	肚子，胃
...
Shipped them off all cozy and `snug` to a	舒适的，温暖的
...
What? Well, where `the dickens` are they?	（用于问句中，表示烦恼或吃惊）究竟
...
No time to kiss the `Blarney stone`, boys.	巧言石（爱尔兰布拉尼城堡的石头，相传吻此石头即善于花言巧语）

We need to find intel

on Dave's location, `pronto`.	很快地；马上，立刻

All right, you, where's Dave!

Give us the goods. Spit it out!

\- Don't make me use `shillelagh`!	橡木棍

\- Sir, that's... That's a baby squid.

Sorry, `laddie`.	男孩，小哥

Stranded on the `Emerald` Isle	绿宝石；祖母绿；翠绿色
...
`Begorrah`! Does anyone speak Gaelic?	天啦（来自感叹语by god的变体）
...
\- Sweet `chariot` of the gods!	二轮战车
...
Those `ludicrous` butterballs	滑稽的；荒唐的

are letting David escape!

Eva, secure transport.

Short Fuse, Corporal, `hitch` us a ride.	搭便车，搭免费车（来自`hitchhike`）
...
\- Have you `purged` the chemical toilet?	清除，净化
...
while we have been cast `adrift`	漂流的，随波逐流的
...
It's five `klicks` southwest,	公里（东南亚国家用语）
...
a `homing device` on Private?	自动引导装置，制导装置

All of you, actually, when I darted you.

You low-down,

dirty, `mangy`, filthy,	污秽的，肮脏的
...
`Behold`, as I unleash the full power		看，注视（see with attention）
...
I don't want to die all `squishy`!	粘糊糊的；湿软的
