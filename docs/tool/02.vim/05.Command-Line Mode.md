...
- `<C-w>` deletes backward to the start of the line
...
In general, we coudl say that a range takes this form: `:{start},{end}`
...
The `:copy` command (and its shorthand `:t`) lets us duplicate one or more lines from one part of the document to another, while the `:move` command lets us place them somewhere selse in the document.
...
- `:%normal i//` commment out an entire JavaScript file
...
- `@:` repeast last Ex command in Command-Line mode
